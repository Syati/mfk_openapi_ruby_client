...
module MfkOpenapiRubyClient
  class CustomerApi
    ...
    # @param customer_id [String] 対象の顧客IDを指定してください。
    # @param [Hash] opts the optional parameters
    # @return [Array<(BankTransfer, Integer, Hash)>] BankTransfer data, response status code and response headers
    ...
    # @option opts [Time] :created_at_from 指定された日時以降に作成された顧客を取得します。指定された日時に作成されたものも含まれます。 RFC3339の&#x60;date-time&#x60;(2019-04-01T10:36:43%2B09:00)で指定してください。
    # @option opts [Time] :created_at_to 指定された日時以前に作成された顧客を取得します。指定された日時に作成されたものも含まれます。 RFC3339の&#x60;date-time&#x60;(2019-04-01T10:36:43%2B09:00)で指定してください。
    ...
    # @option opts [String] :before 任意のリソースIDを指定します。追加日時の降順でこのIDのリソースよりも前のリソースを取得します。この時&#x60;before&#x60;で指定したIDのリソースは結果に含まれません。
    # @option opts [Integer] :limit 取得したいリソースの最大件数を指定します。1~200の間の整数で指定してください。指定がない場合は20になります。
    # @return [Array<(CustomersList, Integer, Hash)>] CustomersList data, response status code and response headers
    ...
    # 顧客更新
    # @param customer_id [String] 対象の顧客IDを指定してください。
    # @param payload [CustomerUpdatePayload] 顧客更新情報です。
    # @param [Hash] opts the optional parameters
    # @return [Customer]
