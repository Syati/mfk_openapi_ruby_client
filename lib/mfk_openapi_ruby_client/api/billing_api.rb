...
module MfkOpenapiRubyClient
  class BillingApi
    ...
    def get_billing_with_http_info(billing_id, opts = {})
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'Billing'
    ...
    # @option opts [String] :customer_number 顧客番号を指定します。指定された顧客の請求を取得します。
    ...
    # @option opts [Date] :due_date_from 日付を指定します。指定した日付以降の支払期限となっている請求を取得できます。指定された日付のものも含まれます。
    # @option opts [Date] :due_date_to 日付を指定します。指定した日付以前の支払期限となっている請求を取得できます。指定された日付のものも含まれます。
    # @option opts [Date] :issue_date_from 日付を指定します。指定した日付以降の請求書発行日となっている請求を取得できます。指定された日付のものも含まれます。
    # @option opts [Date] :issue_date_to 日付を指定します。指定した日付以前の請求書発行日となっている請求を取得できます。指定された日付のものも含まれます。
    ...
    # @option opts [String] :before 任意のリソースIDを指定します。追加日時の降順でこのIDのリソースよりも前のリソースを取得します。この時&#x60;before&#x60;で指定したIDのリソースは結果に含まれません。
    # @option opts [Integer] :limit 取得したいリソースの最大件数を指定します。1~200の間の整数で指定してください。指定がない場合は20になります。
    # @return [Array<(BillingsList, Integer, Hash)>] BillingsList data, response status code and response headers
    def get_billings_list_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && !opts[:'customer_number'].nil? && opts[:'customer_number'].to_s.length > 100
        fail ArgumentError, 'invalid value for "opts[:"customer_number"]" when calling BillingApi.get_billings_list, the character length must be smaller than or equal to 100.'
      end
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'BillingsList'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['apikey']

      new_options = opts.merge(
        :operation => :"BillingApi.get_billings_list",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
