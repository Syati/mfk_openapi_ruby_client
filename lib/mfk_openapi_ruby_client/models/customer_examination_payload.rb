...
module MfkOpenapiRubyClient
  ...
  class CustomerExaminationPayload
    ...
    # 与信枠審査に利用できるその他情報を記載できます。<br>Sandbox環境において与信枠審査で任意の結果を指定するためにこの値を用います。詳しくは[審査結果の操作](#environment_magic_number)を参照ください。
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MfkOpenapiRubyClient::CustomerExaminationPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MfkOpenapiRubyClient::CustomerExaminationPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @amount > 150000000
    ...
    def amount=(amount)
      ...
      if amount > 150000000
        fail ArgumentError, 'invalid value for "amount", must be smaller than or equal to 150000000.'
      end
